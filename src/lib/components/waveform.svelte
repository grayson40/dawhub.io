...
<div class="relative w-full rounded-lg overflow-hidden border border-border bg-card">
  {#if isLoading}
    <div class="h-32 bg-accent/10 animate-pulse" />
  {:else}
    <button
      type="button"
    ...
    >
      {#if peaks.length > 0 && containerWidth}
        ...
        <!-- Progress wave -->
        <div
          class="absolute top-0 left-0 overflow-hidden transition-all duration-100"
      ...
      <div class="absolute left-4 top-1/2 -translate-y-1/2">
        <Button
          variant="outline"
          size="icon"
          class="h-8 w-8 rounded-full bg-background/80 backdrop-blur hover:bg-background/90 transition-colors"
