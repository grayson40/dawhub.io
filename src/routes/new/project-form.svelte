...
<script lang="ts">
  ...
  // Available genres
  const genres = [
    'Electronic', 'Hip Hop', 'Pop', 'Rock', 'R&B', 'Jazz', 'Classical', 'Folk',
    'Country', 'Metal', 'Blues', 'Latin', 'Reggae', 'World', 'Other'
  ];
...
<form method="POST" class="space-y-8" use:enhance enctype="multipart/form-data">
  ...
  <Form.Field {form} name="genre">
    <Form.Control let:attrs>
      ...
      <Select.Root
        onSelectedChange={(s) => s && ($formData.genre = String(s.value))}
      >
  ...
  <Form.Field {form} name="bpm">
    ...
    <Form.Description>
      The tempo of your track in beats per minute.
    </Form.Description>
    <Form.FieldErrors />
  </Form.Field>
  ...
  <Form.Field {form} name="key">
    <Form.Control let:attrs>
      ...
      <Select.Root
        onSelectedChange={(s) => s && ($formData.key = String(s.value))}
      >
        <Select.Trigger {...attrs}>
